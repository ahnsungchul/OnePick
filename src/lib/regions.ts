...
// 시/군/구별 읍/면/동 데이터 (주요 지역 외에는 샘플 데이터로 대체)
export const dongData: Record<string, Record<string, string[]>> = {
  '서울': {
    '강남구': ['전체', '개포동', '논현동', '대치동', '도곡동', '삼성동', '세곡동', '수서동', '신사동', '압구정동', '역삼동', '일원동', '자곡동', '청담동'],
    '서초구': ['전체', '내곡동', '반포동', '방배동', '서초동', '신원동', '양재동', '염곡동', '우면동', '원지동', '잠원동'],
    '송파구': ['전체', '가락동', '거여동', '마천동', '문정동', '방이동', '삼전동', '석촌동', '송파동', '신천동', '오금동', '잠실동', '장지동', '풍납동'],
    '전체': ['전체']
  },
  '경기': {
    '성남시': ['전체', '분당동', '수내동', '정자동', '율동', '서현동', '이매동', '야탑동', '판교동', '삼평동', '백현동', '금곡동', '구미동', '운중동', '대장동', '신흥동', '태평동', '수진동', '단대동', '산성동', '양지동', '복정동', '위례동', '성남동', '중앙동', '금광동', '은행동', '상대원동', '여수동', '도촌동', '갈현동', '하대원동'],
    '전체': ['전체']
  },
  '전국': {
    '전체': ['전체']
...
// 동 데이터 가져오기 헬퍼 함수
export const getDongs = (sido: string, sigungu: string): string[] => {
  if (sido === '전국' || sigungu === '전체') return ['전체'];
  
  if (dongData[sido] && dongData[sido][sigungu]) {
    return dongData[sido][sigungu];
  }
  
  // 데이터가 없는 지역의 경우 샘플 데이터 반환
  return ['전체', '1동', '2동', '3동', '읍/면/리', '상세 지역'];
};
