...
interface MapEstimateFullModalProps {
  estimateId: string | null;
  isOpen: boolean;
  onClose: () => void;
}

export default function MapEstimateFullModal({ estimateId, isOpen, onClose }: MapEstimateFullModalProps) {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [id, router, cat, prv, cit, userId, isOpen]);
  ...
  const handleBidSubmit = async (e?: React.FormEvent, bypassConflict = false) => {
  ...
  if (!isOpen || !estimateId) return null;
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 !m-0">
      ...
      <div className="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-50 shrink-0">
          <h2 className="text-lg font-black text-slate-800">요청 상세 및 견적 작성</h2>
          <div className="flex items-center gap-2">
            ...
            <button 
              onClick={onClose}
              className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-xl transition-all ml-2"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className={`text-xs font-bold px-4 py-1 rounded-full shadow-md ${
                (estimate.status === 'PENDING' || estimate.status === 'BIDDING') ? 'bg-blue-600 text-white' :
                estimate.status === 'SELECTED' ? 'bg-emerald-500 text-white' :
                estimate.status === 'IN_PROGRESS' ? 'bg-blue-600 text-white' :
                'bg-slate-500 text-white'
              }`}>
                {(estimate.status === 'PENDING' || estimate.status === 'BIDDING') ? '요청중' :
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                          ...
                          <div className="flex gap-1">
                            <select 
                              className="w-2/3 px-3 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm font-medium"
                            ...
                            <select 
                              className="w-1/3 shrink-0 px-3 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm font-medium"
                ...
                <div className="lg:col-span-2 bg-white rounded-3xl p-5 border border-slate-100 shadow-sm">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100 flex justify-between items-end">
                      <div>
                        <p className="text-xs font-bold text-slate-400 uppercase mb-0.5">총 견적 합계</p>
                      </div>
                      <div className="text-right">
                        <span className="text-xl font-black text-blue-600">
                          {bidItems.reduce((sum, item) => sum + (parseInt(item.amount) || 0) * 10000, 0).toLocaleString()}
                        </span>
                ...
                <div className="bg-slate-900 rounded-3xl p-5 text-white relative overflow-hidden group gap-4 h-full flex flex-col min-h-[160px]">
                  ...
                  <div className="flex-1 flex flex-col justify-center items-center relative z-10">
                    <div className="flex items-baseline gap-1">
                      <span className="text-3xl font-black">{bidItems.reduce((sum, item) => sum + (parseInt(item.amount) || 0) * 10000, 0).toLocaleString()}</span>
      ...
      {conflictData && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col max-h-[80vh]">
            <div className="p-6 md:p-8 flex-1 overflow-y-auto">
              ...
              <p className="text-sm text-slate-500 text-center mb-6">
                그래도 무시하고 일정을 추가하시겠습니까? (겹치는 날짜: <span className="font-bold text-slate-700">{conflictData.conflicts.join(', ')}</span>)
              </p>
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3 shrink-0">
              ...
              <button 
                onClick={() => {
                  ...
                  handleBidSubmit(undefined, true); // 무시하고 추가
                }}
                disabled={isSubmittingBid}
